...
JSON is the canonical, complete analysis export and includes the resolved baseline, derived baseline, provenance, warnings, active inputs, overrides, and complete projection. It declares that it is share-safe and built by a typed allowlist. Account references are replaced consistently with deterministic planner-type aliases such as `TFSA 1` and safe keys such as `tfsa_1`; every other source record receives an export-local alias such as `event_1`, `recurring_expense_1`, or `category_1`.
...
Both formats are anonymized by default and contain no raw source-system record IDs, Lunch Money account identifiers, real account names, account numbers, credentials, or user-controlled free text. Event labels, recurring descriptions, warnings, connection messages, payees, merchants, addresses, and notes are genericized while their analytical amounts, dates, classifications, directions, codes, severities, and safe references remain available.
