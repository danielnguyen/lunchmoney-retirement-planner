...
The primary configuration uses account roles and named savings plans. Account identity and roles live under `accountMappings`; simple room, employment RRSP assumptions, and savings policy contain no account route IDs. An optional liability payment matcher repeats only its exact source account reference because that source is transaction evidence rather than a projection route. The baseline compiler produces the detailed resolved account phases, global room model, waterfall, surplus input, automatic taxable account when required, and policy discriminator. Simple and advanced configuration cannot mix.
...
Cash-flow audit evidence records the aggregate category/account contribution to each derived value without retaining raw transactions. Debt-payment evidence selected by a dedicated category or an exact payee/source matcher is excluded before ordinary spending, linked to a liability role, and marked as replaced by the future schedule. Unrelated records in a shared category remain ordinary spending. Imported residence value/date records remain distinct from financial-account balances. Lifestyle-spending phases retain their explicit multipliers and source separately from the live essential and discretionary baselines, so explanations can show baseline × multiplier × inflation without inventing a fixed configured amount. Lunch Money amounts and names remain distinguishable from local YAML assumptions, compatibility fallbacks, temporary browser overrides, Canadian references, and projection output. Raw matcher payees and source identifiers are not report fields. The baseline schema is `1.8`; the projection and export schemas are `9.0`.
