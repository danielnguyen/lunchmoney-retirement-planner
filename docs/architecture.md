...
Both normal export routes always create one deterministic anonymization context per request; no raw, private, or opt-out export path exists. A typed, allowlisted transformation constructs the export model field by field and never recursively copies the baseline or projection request. Included accounts, employment phases, contribution phases, events, recurring expenses, categories, warnings, and unmapped records receive generic export-local aliases based only on type and encounter order. Those aliases are applied consistently to baseline inputs, active inputs, projection inputs, account-balance maps, annual phase labels, provenance, overrides, contributions, and targets.
