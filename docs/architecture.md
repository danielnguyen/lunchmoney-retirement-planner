...
At projection start, configured room is used without adding the current year again. Later January boundaries add the dated or forecast TFSA limit, restore prior-year TFSA withdrawals, and generate RRSP room from explicit prior-year eligible earned income, pension adjustment, other reduction, and the applicable cap. Planned routes execute in configured order before surplus routing. Registered deposits consume shared room immediately; cash-funded transfers remain asset-neutral, while actual income-withheld deposits remain external bridge additions.
