...
After returns, income, spending, tax, contribution phases, and events, each targeted event inflow is deposited only into its own explicit target and removed from unassigned policy cash. Positive unassigned cash refills the explicit indexed reserve account, then either remains in that account or moves to the configured non-registered destination. No account-order lookup selects a reserve or destination. TFSA, RRSP/RRIF, cash, and debt accounts cannot be automatic excess destinations until registered-account room is modelled.
