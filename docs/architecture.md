...
The scenario-draft route shares the pure scenario-control inventory with the browser without importing React into server code. Every constructed control must resolve one explicit classification: a deterministic config binding, a `live_baseline` conversion requiring a user decision, or scenario-only with a user-facing reason. Bindings contain application-owned scalar locators and stable sequence identities; the request contains only the current YAML draft, active content version, validated resolved baseline, numeric overrides, and the preview/apply decision. Unknown keys, non-finite or out-of-range values, arbitrary fields, stale versions, ambiguous identities, aliases, and unsafe YAML constructs fail before patching. No caller-provided path or YAML locator is accepted.

Preview classifies without mutating text. Apply finds existing scalar source ranges, builds non-overlapping edits, applies them from the end of the document, and then reparses through the authoritative planner-config validator. This preserves comments, order, quoted keys, unrelated whitespace, and pre-existing manual edits without generic YAML reserialization. Apply returns an in-memory draft and never writes; only the separately opted-in current-config `PUT` can persist it.
...
The dashboard owns the loaded config document, raw YAML draft, content version, dirty/validation/save status, and scenario-application summary so drawer lifecycle cannot discard manual edits. Applying a scenario previews classifications first and, after any combined live-source decision, updates only this shared draft. Absolute live spending and imported residence balances remain scenario-only. Applying never clears overrides and never saves. A successful explicit save reloads the baseline, clears overrides and the applied summary, clears stale projection state, and regenerates the projection. Save/validation conflicts preserve draft, overrides, and summary. A failed post-save reload installs the blocking error, removes the old projection, retains repair context, and keeps the config editor available from the blocking state.
