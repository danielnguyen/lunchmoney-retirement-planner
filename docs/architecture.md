...
Both normal export routes always create one deterministic anonymization context per request; no raw, private, or opt-out export path exists. A typed, allowlisted transformation constructs the export model field by field and never recursively copies the baseline or projection request. Imported and projection-only accounts, employment phases, contribution phases, events, recurring expenses, categories, warnings, and unmapped records receive generic export-local aliases based only on type and encounter order. Those aliases are applied consistently to baseline inputs, active inputs, projection inputs, surplus and contribution-waterfall references, account-balance, contribution-detail and allocation maps, annual phase labels, provenance, overrides, and targets.
...
Projection schema `7.0` models one person, explicit resolved employment-income phases, per-account contribution phases, global TFSA and RRSP room pools, a per-source contribution waterfall, account origin, a mandatory resolved surplus policy, concrete CPP/OAS inputs, optional future events, and simplified tax assumptions. Configuration discriminators and account references are resolved at the baseline boundary. The calculation result contains shared government-benefit, surplus-allocation, registered-room, annual route, and per-account contribution details consumed by the dashboard, presentation, explanation, and export layers.

At projection start, configured room is used without adding the current year again. A January start requires zero pre-projection RRSP-generation totals. Later January boundaries add the dated or forecast TFSA limit, restore prior-year TFSA withdrawals, and generate RRSP room from explicit prior-year eligible earned income, pension adjustment, other reduction, and the applicable cap. RRSP generation is mandatory whenever an RRSP/RRIF destination is reachable through a canonical, compatibility, overflow, or active surplus route; omitted values never resolve silently to zero. Planned routes execute in configured order before surplus routing. Registered deposits consume shared room immediately; cash-funded transfers remain asset-neutral, while actual income-withheld deposits remain external bridge additions.
...
The engine also accumulates nominal and real `financialAssetsBridge` records from opening financial assets to the exact retirement snapshot. Employment cash, public benefits and pension, future-event inflows, income-withheld contributions, actual non-debt returns, spending, one-time outflows, and taxes are recorded from the same monthly loop. Cash-funded contributions and surplus routing are excluded as additive bridge terms because they transfer value between financial accounts. Shared actual-contribution totals equal planned-route allowed deposits plus surplus-funded deposits; all surplus-funded deposits are cash funded and are reported once without a second cash deduction. Routing can change composition, and later account-specific returns can change future total assets. Projection calculation fails if either bridge or any contribution, registered-room, monthly, or annual surplus equation misses reconciliation by more than one cent.
