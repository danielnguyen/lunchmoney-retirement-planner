...
Browser controls materialize temporary inputs from the current baseline and call the projection API. Each control declares its financial semantics: ages use range inputs, while currency, percentage, and other numeric values use precise number inputs. Percentage-point values are converted explicitly to the decimal rates used by the projection domain. Owner-facing labels cover savings, room, reserve, residence value/appreciation, and amortizing-liability rate/payment assumptions; structural choices remain configuration-only. A field reset removes one override. Reset all removes every override. Refresh requests a new baseline and clears overrides. Saving YAML also reloads the baseline before clearing overrides; scenario values are never written into configuration.
