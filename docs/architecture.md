...
Included debts are resolved separately from financial accounts. A positive debt requires an explicit amortizing or payoff-at-start treatment; a zero balance may remain zero. A configured residence produces a typed non-financial asset, and the optional unique `primary_mortgage` role links it to an amortizing liability. Legacy zero debt return/allocation/priority fields are accepted only as a deterministic migration boundary; non-zero investment assumptions or a silent static positive debt are rejected.
...
The baseline API schema is `1.6`. Its `cashFlowAudit` structure groups mapped transactions by category and account for income, non-debt essential spending, non-debt discretionary spending, and historical debt payments; records resolved contribution values by account, funding mode, and source; and retains normalized reviewed recurring-item context. Imported financial accounts and liabilities both retain Lunch Money provenance, while a configured residence and automatic taxable account retain projection-configuration provenance. Resolved inputs contain distinct financial-account, non-financial-asset and liability collections plus the policy, room, waterfall and benefit models.
...
Required liability payments are resolved in the monthly cash-flow sequence before simple cash-funded savings. An amortizing liability applies interest, reduces the final regular payment to the exact amount due, then applies dated lump sums without allowing a negative balance. The full payment leaves financial assets; interest is consumption; principal lowers financial assets and the liability together. Payoff-at-start treatment clears the opening liability in month one. Payments cease automatically at payoff, and home equity is never offered to withdrawal logic.
