...
Private assumptions and mappings are loaded from `PLANNER_CONFIG_PATH`, which defaults to `config/planner.local.yaml`. YAML is the canonical human-maintained format and allows opaque Lunch Money identifiers to carry nearby account/category comments. The loader selects YAML or JSON from the configured file extension; `.yaml` and `.yml` are supported, while `.json` remains available for an explicitly configured compatibility path. All private local artifacts are ignored by Git and the Docker build context.

The dedicated current-config API exposes only the configured YAML file; callers cannot select a path. Reading and validation are available independently of writing. Writes require the exact `PLANNER_CONFIG_WRITE_ENABLED=true` opt-in and validate through the same parser and planner validator as normal startup loading. The service prepares same-directory temporary files for both the submitted text and prior-text backup, then performs a final SHA-256 version check before either rename. A detected conflict changes neither active config nor backup and cleans both temporary files. After the check, backup replacement precedes active replacement so backup failure leaves the active file untouched and active-replacement failure leaves the previous config recoverable from the backup. The two renames are intentionally not described as one filesystem transaction. Submitted YAML text is retained verbatim so comments and formatting survive. Responses expose only a safe filename and never expose environment values, tokens, or arbitrary filesystem details. The route is dynamic, uncached, and fixed to the Node.js runtime.

The scenario-draft route shares the pure scenario-control inventory with the browser without importing React into server code. Every constructed control must resolve one explicit classification: a deterministic config binding, a `live_baseline` conversion requiring a user decision, or scenario-only with a user-facing reason. Resolution uses the supplied YAML draft's validated mode, optional blocks, configured source modes, and stable sequence identities rather than assuming the active baseline still matches a manually edited draft. Preview source-resolves ordinary scalar destinations as well, so predictable mode, structure, and identity mismatches become actionable scenario-only results while unsupported YAML aliases remain explicit apply-time failures. Bindings contain only application-owned scalar locators and safe display metadata; the request contains only the current YAML draft, active content version, validated resolved baseline, numeric overrides, and the preview/apply decision. Unknown keys, non-finite or out-of-range values, arbitrary fields, stale versions, ambiguous identities, aliases, and unsafe YAML constructs fail before patching. No caller-provided path or YAML locator is accepted.

Preview classifies without mutating text and records the active resolved baseline separately from every destination's actual current YAML scalar or `live_baseline` sentinel. Application-owned display names make multi-destination return assumptions reviewable without exposing browser-supplied paths or source-system identifiers. Apply finds existing scalar source ranges, canonicalizes numeric replacements to deterministic non-scientific decimal text, builds non-overlapping edits, applies them from the end of the document, and then reparses through the authoritative planner-config validator. This removes insignificant binary floating-point noise while preserving meaningful planner precision, comments, order, quoted keys, unrelated whitespace, and pre-existing manual edits without generic YAML reserialization. Apply returns an in-memory draft and never writes; only the separately opted-in current-config `PUT` can persist it.
...
Projection schema `9.0` models one person, financial accounts, non-financial assets, explicit liabilities, employment, contribution, and lifestyle-spending phases, global TFSA and RRSP room pools, a resolved surplus policy, concrete CPP/OAS inputs, optional future events, and simplified tax assumptions. Each projected month selects the spending phase from working age, multiplies the separate live essential and discretionary baselines, and then applies the shared inflation factor. The same monthly engine produces account balances, residence appreciation, liability schedules, cash flow, retirement-funding assets, home equity and total net worth. Dashboard, presentation, explanations and exports consume that shared typed result.
...
Browser controls materialize temporary inputs from the current baseline and call the projection API. Each control declares its financial semantics: ages use range inputs, while currency, percentage, and other numeric values use precise number inputs. Number controls own a string draft so empty and transitional edits do not alter domain inputs; complete finite values commit only inside the configured range, and invalid drafts restore the active value on blur. Percentage-point values convert explicitly to decimal rates only when committed. Reset and external baseline changes synchronize drafts without replacing an in-progress draft during unrelated renders. Owner-facing labels cover savings, room, reserve, residence value/appreciation, and amortizing-liability rate/payment assumptions; structural choices remain configuration-only. Each control also presents its refreshed source distinctly from a temporary scenario value. A field reset removes one override. Reset all removes every override. Refresh requests a new baseline and clears overrides.

The dashboard owns the loaded config document, raw YAML draft, content version, draft revision, dirty/validation/save status, scenario-application summary, and one configuration-drawer state. That drawer opens guided scenario controls by default and changes its view in place to the advanced YAML editor; the stable dialog identity, opener, temporary overrides, and unsaved draft survive view changes. The blocking state opens the same drawer identity directly in YAML view because controls cannot be constructed without a baseline.

Applying a scenario previews classifications first and, after any combined live-source decision, updates only this shared draft before changing the open drawer to YAML view. The summary reports the actual YAML before-values and records the application revision; later manual edits mark it as the last scenario application rather than an authoritative description of the current draft. Another successful application refreshes it, and Revert clears it. Absolute live spending and imported residence balances remain scenario-only. Applying never clears overrides and never saves. A successful explicit save reloads the baseline, clears overrides and the applied summary, clears stale projection state, and regenerates the projection. Save/validation conflicts preserve draft, overrides, summary, and the YAML view. A failed post-save reload installs the blocking error, removes the old projection, retains repair context, and keeps the unified drawer open in YAML view for repair.
