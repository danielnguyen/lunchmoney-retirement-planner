...
The dashboard owns the loaded config document, raw YAML draft, content version, draft revision, dirty/validation/save status, and scenario-application summary so drawer lifecycle cannot discard manual edits. Applying a scenario previews classifications first and, after any combined live-source decision, updates only this shared draft. The summary reports the actual YAML before-values and records the application revision; later manual edits mark it as the last scenario application rather than an authoritative description of the current draft. Another successful application refreshes it, and Revert clears it. Absolute live spending and imported residence balances remain scenario-only. Applying never clears overrides and never saves. A successful explicit save reloads the baseline, clears overrides and the applied summary, clears stale projection state, and regenerates the projection. Save/validation conflicts preserve draft, overrides, summary, and its stale indication. A failed post-save reload installs the blocking error, removes the old projection, retains repair context, and keeps the config editor available from the blocking state.
