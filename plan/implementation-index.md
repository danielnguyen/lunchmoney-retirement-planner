...
- Synthetic validation: Passed locally — 85 focused Plan details, dashboard, explanation, typography, and print tests across 3 files; 105 focused export/privacy/runtime/configuration tests across 6 files; 579 tests across the complete 32-file suite; typecheck, lint, production build, diff check, Docker image build, Compose validation, isolated schema-aware health smoke with the public synthetic example (`5.0` baseline schema and `13.0` projection schema), and clean container removal. Exact live-head hosted CI and Docker results are retained in the pull-request description. No private planner configuration or values were accessed.
