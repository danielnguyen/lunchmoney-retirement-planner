...
In simple mode, explicit savings plans are processed first. Workplace RRSP runs before personal savings and the reserve-building plan. The reserve plan compares the combined reserve-member balance after returns with the indexed target, retains only the funded shortfall in the refill account, and sends a same-month crossing amount through the personal order. The explicit unplanned-cash policy then either retains remaining positive cash in operating cash or funds the indexed operating and combined reserve targets before sweeping true excess through the personal order. Account ordering never selects a route.
...
The baseline API schema `1.8` includes aggregate cash-flow audit evidence, debt-payment replacement evidence, typed imported non-financial-asset balances, distinct financial accounts/non-financial assets/liabilities, resolved employment, savings, and lifestyle-spending phase provenance, concrete CPP/OAS inputs, projection-only account provenance, surplus-policy provenance, registered-room provenance, and resolved waterfall routes:
