...
The draft operation preserves comments, key order, quoted mapping keys, unrelated whitespace, and existing unsaved manual edits. Closing and reopening the config drawer retains the draft; **Revert changes** intentionally replaces it with the latest file from disk. Each applied or skipped value is summarized above the editor until a successful save and baseline reload.
